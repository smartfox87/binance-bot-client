    ...
    <section className="container flex flex-col gap-4 py-4">
      ...
      <ul>
        <li className="flex border-y border-gray-300 py-2">
          ...
          <div className="w-[1%] grow border-r border-gray-300 px-2">Side</div>
          ...
          <div className="w-[3%] grow border-r border-gray-300 px-2">Distance</div>
          <div className="w-[3%] grow border-r border-gray-300 px-2">Avg price</div>
          <div className="w-[3%] grow border-r border-gray-300 px-2">Stop price</div>
          <div className="w-[2%] grow border-r border-gray-300 px-2">Qty</div>
          <div className="w-[2%] grow border-r border-gray-300 px-2">Exec. qty</div>
          <div className="w-[3%] grow border-r border-gray-300 px-2">Result</div>
        ...
        {ordersData.map(({ symbol, id, status, type, side, price, avg_price, stop_price, orig_qty, executed_qty, result, update_time }) => (
          <li key={id} className="flex border-b border-gray-300 py-2">
            ...
            <div className="w-[3%] grow border-r border-gray-300 px-2">
              {status !== "CANCELED" && symbolsData[symbol] ? ((Math.abs(price - symbolsData[symbol][1]) / symbolsData[symbol][1]) * 100).toFixed(5) : 0} %
            </div>
            <div className="w-[3%] grow border-r border-gray-300 px-2">{formatPrice(avg_price, "auto")} $</div>
            <div className="w-[3%] grow border-r border-gray-300 px-2">{formatPrice(stop_price, "auto")} $</div>
            <div className="w-[2%] grow border-r border-gray-300 px-2">{formatPrice(orig_qty, "auto")}</div>
            <div className="w-[2%] grow border-r border-gray-300 px-2">{formatPrice(executed_qty, "auto")}</div>
            <div className="w-[3%] grow border-r border-gray-300 px-2">{result && formatPrice(result, "auto")} $</div>
